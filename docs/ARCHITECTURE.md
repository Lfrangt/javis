...
- Knowledge vault lane: Obsidian/Markdown vault discovery, read-only note search with snippets/tags/wikilinks, preview-first note creation/append/daily-note plans, read-only MCP server discovery, preview-only MCP workflow planning, local MCP execution approval requests from local JSON configs, approved stdio `tools/list` schema inspection, separately approved stdio `tools/call` execution, confirmed Markdown writes through file policy, `/api/knowledge/*`, `/api/mcp/servers`, `/api/mcp/workflow`, `/api/mcp/tool-call`, CUI benchmarks/discovery/previews, and Realtime tools.
