...
- Realtime dogfood preparation lane: when live evidence is blocked on `needs_live_session`, `/api/work/next` can prepare the full live drill without opening the microphone by loading the prompt script, starting or reusing the operator tracker, saving prep/archive evidence, and returning the final mic-confirmed command for the user-run renderer path.
