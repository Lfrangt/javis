...
- Creative app workflow lane: recognizes video editing and music composition requests, picks likely creative software, records stage action packs for imports, timeline edits, subtitles, MIDI sketches, mix/export previews, and executes one guarded action at a time through app workflow, observe, file workflow, UI planning, or current-app control.
