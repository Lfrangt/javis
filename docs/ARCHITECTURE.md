...
- Knowledge vault lane: Obsidian/Markdown vault discovery, read-only note search with snippets/tags/wikilinks, preview-first note creation/append/daily-note plans, read-only MCP server discovery from local JSON configs, confirmed Markdown writes through file policy, `/api/knowledge/*`, `/api/mcp/servers`, CUI benchmarks/discovery, and Realtime tools.
- MCP discovery lane: scans known local Claude Desktop, Claude Code, Cursor, and project `.mcp.json` files without starting server commands. It returns sanitized server names, source config paths, transport, command basename or URL host, args count, and env key names only; env values and URL queries stay redacted.
