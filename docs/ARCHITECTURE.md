...
Electron process
  Local Express API
  OpenAI spend firewall and one-request lease store
  Global pet hotkey
...
- OpenAI spend firewall lane: all OpenAI Responses and Realtime provider calls go through the resident spend guard. Defaults are zero-spend: hard lock on, cloud mode off, daily budget 0, unattended budget 0, autopilot cloud disabled, renderer startup probes disabled, egress guard on, and a short-lived one-request spend lease required even after phrase confirmation. `/api/openai/spend-sentinel` is a resident local-only watcher over that guard, while `/api/openai/zero-spend-lockdown` adds a current-process emergency lock that clears active leases and blocks later spend checks immediately before `.env` reload/restart. The same lane exposes local forensics over allowed/blocked event sources so quota questions can be answered without calling OpenAI.
...
- Zero-spend quick answer lane: a deterministic whitelist handles narrow local quick replies while cloud spend is locked, covering identity, capability, local time/date, one-sentence acknowledgement, and spend status. It never reads private content or answers open-ended knowledge questions; unsupported quick questions remain held or are routed to background/workbench lanes.
...
- Local command context ownership: deterministic local commands that already read their own evidence, such as latency, voice status, blockers, approvals, browser/app read paths, learning, and work-next, can skip pre-route screen/UI context assembly even when fallback renderer calls include screen flags. Screen and UI observation commands still gather their context inside their own guarded tool path.
- Lightweight observe split: natural screen/window observation defaults to Mac/screen/browser/task metadata without an Accessibility tree, while button/control questions route to the app UI lane. This keeps "what is on my screen?" fast and reserves AX reads for tasks that actually need UI structure.
- App UI cache lane: Accessibility outline reads keep a small in-memory LRU keyed by app/window, plus the latest slot for compatibility. This lets local voice questions such as "当前窗口有哪些按钮" reuse a recent bounded AX outline even if ambient prewarm briefly observed another app/window, while still falling back to a fresh read when the requested current window is not cached.
...
- Work-session voice lane: natural local voice phrases such as `记到当前会话：...` and explicit `voice:chat` commands `/session start <goal>`, `/note <text>`, and `/session end [note]` operate on the local work-session store. The lane records recoverable task context without microphone capture, Realtime sessions, Terminal windows, screen capture, clipboard text, app actions, or user-file mutation.
- Incident report lane: `/api/incident/report`, CUI `IR`, local `voice:chat` `/incident`, and natural local voice questions like "谁干的/为什么开这么多窗口" read the recent local audit tail for window, Terminal, Realtime, resident, worker, approval, and desktop-pet events. This lane is explanation-only: it returns compact metadata evidence and current voice/window state without screenshots, clipboard text, browser page text, full Accessibility trees, microphone capture, Realtime sessions, Terminal windows, or action execution.
- Voice latency lane: `/api/voice/latency`, CUI `VL`, `npm run config -- --print-voice-latency`, local `voice:chat` `/latency`, and natural local voice questions like `语音延迟怎么样` read sanitized voice-command audit timing metadata. This lane reports avg/p90/p95 latency, likely bottleneck, slow turns, and next optimization hints without microphone capture, Realtime sessions, screen capture, clipboard text, browser page text, full Accessibility trees, Terminal windows, action execution, or user-file mutation.
- Voice standby prompt-pack lane: `/api/voice/standby`, `/api/pet/status`, wake handoff, the terminal CUI, local `/voice`, local `/try`, natural `prompt_suggestions`, and the compact compose placeholder share one short `promptPack` with the next safe utterance, examples, and no-mic/no-Terminal safety flags. They also share a compact `inputMode` contract for `typed_local_intake` (`micDefault=off`, keyboard or macOS dictation, Enter to send), so fallback surfaces do not imply that live audio is recording. The examples are ranked from existing local metadata: resumable voice routes, active/blocked work, browser activity, cached current-app UI, capability status, and local distillation. This keeps the desktop surface actionable while the detailed provider recovery and routing diagnostics stay in CUI/API.
...
- Resident health watchdog lane: `com.haoge.javis.watchdog` checks the public local `/api/health` endpoint every minute, kickstarts the resident LaunchAgent when the process is alive but the API stops responding, and exposes its state through resident/setup/overnight/doctor/CUI surfaces plus local voice `resident_health` questions such as `你还活着吗` or `watchdog 开着吗`.
...
- Conversation state lane: renderer-reported connecting/live/idle/error voice lifecycle with heartbeats plus an effective `microphone` state. `micMode` remains the requested Realtime mode, while `microphone.state` reports the actual resident posture (`off`, `armed`, or `open`) so idle fallback cannot be mistaken for live listening.
...
- Realtime renderer watchdog lane: the resident checks raw renderer conversation state on an interval and stops stuck connecting, stale-heartbeat, or overlong live sessions through the renderer control lane. The watchdog is stop-only: it starts no microphone, creates no new Realtime session, opens no Terminal, and keeps status in CUI/API evidence instead of the pet.
...
- Parallel task group lane: routes a bounded set of independent tasks under one `parallelGroup`, preserving per-task owner, scope, lane, status, and result link for progress check-ins. `/api/tasks/parallel/preflight`, CUI `AP`, and `npm run agents:preflight` accept a 20-agent request as a preview-only capacity plan, then report safe concurrency, waves, worker readiness, serialized write conflicts, and blocked lanes without starting workers, opening Terminal, calling OpenAI, writing route records, or touching files. `/api/tasks/parallel` accepts the same large task payload but routes only the selected safe wave by default, preserving original task indexes and returning `executionPlan.remainingParallelBatches`, `serialQueue`, `blocked`, and a persisted `parallelPlan` saved to `parallel-plans.json`; `/api/tasks/parallel/plans/:id/run` resumes later waves by plan id after a restart instead of requiring the user to repeat the task payload.
...
- Classroom/stealth desktop lane: `/api/window/classroom`, `/api/window/hide`, and the menu bar/CUI can hide the desktop pet while leaving the resident API, menu bar, hotkeys, jobs, and monitoring alive. The state persists in `window-state.json`, so class/presentation mode survives resident restarts without adding a visible dashboard.
- Tap-to-summon hotkey lane: global `JAVIS_SUMMON_HOTKEY`/`JAVIS_TAP_HOTKEY` summons JAVIS, parks the capsule at the notch, and opens the compact local `compose` input by default. This path starts no microphone, no Realtime session, no Terminal, and no provider spend even if Realtime later becomes healthy. Direct tap-to-Realtime is behind the explicit `JAVIS_TAP_TO_REALTIME=true` / `JAVIS_SUMMON_REALTIME=true` opt-in plus the normal spend unlocks.
...
- Browser benchmark lane: preview-only fixture checks for summarize, action extraction, form-fill draft redaction, compare/search preview, review-result preview, and research continuation contracts. It uses `/api/browser/benchmarks` and the CUI without opening live pages, executing browser actions, or calling models. Eval/manual-probe browser action execution is blocked unless `JAVIS_EVAL_LIVE_BROWSER_ACTIONS=true` is set for an attended live-browser run.
