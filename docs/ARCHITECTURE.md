...
- Local voice entry lane: `/api/pet/status` exposes a compact `localVoice.interaction` contract whose fallback click opens the local typed intake path instead of Terminal, so the parked capsule can stay quiet when Realtime is blocked. `/api/voice/open-local-loop` remains an explicit CUI/CLI opener for `npm run voice:chat`; it reuses an existing chat loop or debounces a just-opened one so deliberate terminal-loop starts cannot stack windows.
