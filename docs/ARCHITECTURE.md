...
- Resident health watchdog lane: `com.haoge.javis.watchdog` checks the public local `/api/health` endpoint every minute, kickstarts the resident LaunchAgent when the process is alive but the API stops responding, and exposes its state through resident/setup/overnight/doctor/CUI surfaces.
