...
- File organization lane: deterministic by-type folder, batch rename, semantic text-conversion, and copy-convert plans with per-step policy preview, content redaction for generated write plans, explicit apply confirmation, and the same approval/local-execution gates before any move/copy/create/write action.
