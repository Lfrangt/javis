...
- Realtime work-progress lane: while voice is live, sends deduplicated silent `/api/work/progress` updates when background jobs, grouped Codex/Claude/local workers, or workflows change. The `realtime-injection` eval proves grouped worker context is injected as a passive conversation item without `response.create`.
