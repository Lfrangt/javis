...
- OpenAI spend firewall lane: all OpenAI Responses and Realtime provider calls go through the resident spend guard. Defaults are zero-spend: hard lock on, cloud mode off, daily budget 0, unattended budget 0, autopilot cloud disabled, renderer startup probes disabled, egress guard on, and a short-lived one-request spend lease required even after phrase confirmation.
