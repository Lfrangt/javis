...
- Browser research continuation lane: attaches structured next-step actions to browser `research` workflows for unvisited result links, failed pages, and promising follow-up links. These are inert workflow arguments until a later explicit browser workflow call runs them.
