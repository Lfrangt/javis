...
- Knowledge vault lane: Obsidian/Markdown vault discovery, read-only note search with snippets/tags/wikilinks, preview-first note creation/append/daily-note plans, read-only MCP server discovery and preview-only MCP workflow planning from local JSON configs, confirmed Markdown writes through file policy, `/api/knowledge/*`, `/api/mcp/servers`, `/api/mcp/workflow`, CUI benchmarks/discovery/previews, and Realtime tools.
- MCP discovery/planning lane: scans known local Claude Desktop, Claude Code, Cursor, and project `.mcp.json` files without starting server commands. It returns sanitized server names, source config paths, transport, command basename or URL host, args count, env key names only, and a task-to-server preview plan; env values and URL queries stay redacted, and preview planning never calls MCP tools.
