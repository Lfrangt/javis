...
- Local voice loop opener lane: `/api/voice/open-local-loop` opens `npm run voice:chat` in Terminal for quota/auth/provider-blocked states, but first reuses an existing chat loop or debounces a just-opened one so repeated pet clicks cannot stack Terminal windows. `/api/pet/status` exposes a compact `localVoice.interaction` contract whose fallback endpoint is `/api/voice/standby`; that voice-standby primary action currently delegates to `/api/voice/open-local-loop`, so the parked capsule can open the CUI loop without expanding into a dashboard or starting microphone capture.
