...
- Demonstration lane: explicit user-started UI demonstration records with sanitized app/browser/screen/accessibility summaries, deterministic manual-preview playbooks, safe preview-only replay plans, API/CUI/Realtime voice tools, and delete controls. It stores no screenshots or clipboard text.
...
`demonstrations.json` preserves explicit UI demonstrations started by the user through API/CUI/voice. Records store a goal, short user notes, sanitized current-app/browser context, screen metadata, Accessibility outline summaries, and a deterministic manual-preview playbook. Replay planning converts completed records into preview-only app workflow steps that re-observe live UI targets before any later action. They do not store screenshots or raw clipboard text.
