...
- Realtime work-progress lane: while voice is live, sends deduplicated silent `/api/work/progress` updates when background jobs, grouped Codex/Claude/local workers, or workflows change. The renderer reports a sanitized `/api/realtime/progress-injection` receipt into conversation state and audit so active-session dogfood can prove the context was sent without `response.create`.
