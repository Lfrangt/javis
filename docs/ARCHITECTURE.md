...
- Local voice loop opener lane: `/api/voice/open-local-loop` opens `npm run voice:chat` in Terminal for quota/auth/provider-blocked states. `/api/pet/status` exposes a compact `localVoice.interaction` contract so the parked capsule can open that CUI loop without expanding into a dashboard or starting microphone capture.
