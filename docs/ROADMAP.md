...
- Realtime dogfood archive: CUI option `A`, `npm run config -- --save-realtime-dogfood-archive`, `GET/POST /api/realtime/dogfood/archive`, `/api/realtime/dogfood/archives`, and Realtime `save_realtime_dogfood_archive` preserve the current dogfood brief, gap summary, evidence checklist, session tracker, and related audit trail as local JSON without starting microphone capture or storing raw audio.
- Renderer Realtime dogfood trigger: CUI option `R`, `npm run dogfood:realtime-renderer -- --execute --confirm-mic`, and `POST /api/realtime/dogfood/renderer/start` provide an opt-in, mic-confirmed way to make the renderer start the actual WebRTC session, wait for the live data channel, send dogfood prompts, report renderer-stage events, and save a local archive through the script. The renderer listener now survives its own `idle -> connecting -> live/error` state transitions during startup, and the script prints provider quota/rate-limit blockers from `/api/realtime/evidence` instead of only reporting a generic timeout.
- Verifiable next task: run the guided drill through a real renderer/WebRTC voice session with the renderer trigger and CUI evidence monitor open until the drill marks progress answer, handoff tool call, autopilot status tool call, attention explanation tool call, perception consent, UI-demonstration Record & Replay, shortcut list/save/recall/forget, and routed shortcut recall as ready, then save a local dogfood archive for that run.

## Phase 3: Natural Collaboration

- Wake-word or push-to-talk mode.
- Better interruption handling.
- Ongoing screen context with privacy zones: current build supports global private/clear screen mode plus local app/window/browser-host/region privacy rules in `/api/screen/privacy`; app/window/browser-host exclusion rules block screen image use in server-side quick/observe/vision/Realtime preflight model context, while enabled region rules are pixel-masked in the resident process before resident-captured or renderer-posted frames are stored, returned through the API, or injected into Realtime; next, add an interactive region picker and app-specific preset zones.
