...
- File workflows: current build supports list/search/summarize/Q&A, guarded by-type organization plans, batch rename previews, deterministic semantic text-conversion previews for `.txt`/`.md`/`.html`/`.json`/`.csv`/`.tsv`, non-destructive copy-convert previews, redacted generated write plans, explicit `confirm:true` plan apply, and Level 3 create/copy/move/rename/write actions; next, richer app-specific file result checks and binary/document converters.
