...
- Add cancellable background workers with visible logs, structured failure recovery plans, and `/api/jobs/recovery` summaries for attempts, diagnostics, recovery child jobs, and recommended next actions.
...
- Verifiable next task: run the guided drill through a real renderer/WebRTC voice session with the CUI evidence monitor open until the drill marks progress answer, handoff tool call, shortcut list/save/recall/forget, and routed shortcut recall as ready.
