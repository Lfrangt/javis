...
- Verifiable next task: run the guided drill through a real renderer/WebRTC voice session with the CUI evidence monitor and dogfood brief open until the drill marks progress answer, handoff tool call, autopilot status tool call, attention explanation tool call, perception consent, UI-demonstration Record & Replay, shortcut list/save/recall/forget, and routed shortcut recall as ready, then save a local dogfood archive for that run.
