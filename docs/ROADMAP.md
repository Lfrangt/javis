...
- Realtime dogfood operator brief: CUI option `B`, `npm run config -- --print-realtime-dogfood-brief`, and `/api/realtime/dogfood/brief` provide a one-page readiness summary, next prompt, follow-up script, evidence gates, active session state, and start/monitor commands without starting microphone capture.
- Realtime dogfood archive: CUI option `A`, `npm run config -- --save-realtime-dogfood-archive`, `GET/POST /api/realtime/dogfood/archive`, `/api/realtime/dogfood/archives`, and Realtime `save_realtime_dogfood_archive` preserve the current dogfood brief, evidence checklist, session tracker, and related audit trail as local JSON without starting microphone capture or storing raw audio.
