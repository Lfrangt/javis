...
- Realtime voice evidence monitor: current CUI option `V. Watch Realtime voice evidence` watches the real renderer/WebRTC evidence checklist, including structured status/phase/blocker output, recent sanitized Realtime tool-call metadata, shortcut list/candidate/save/forget evidence, and the `/api/realtime/dogfood` manual runbook. `/api/realtime/dogfood/prepare` can manually queue a short local read-only progress sample for live-session progress injection. `/api/work/next` can summon the live-session dogfood path when the blocker is `needs_live_session`; this action is manual-only so unattended autopilot never starts microphone/live voice, and no desktop pet diagnostics are added.
- Verifiable next task: run the same flow through a real renderer/WebRTC voice session with the CUI evidence monitor open until `READY`, ask “后台现在怎么样” to confirm the grouped summary is heard, then ask it to list/save/forget a shortcut phrase and confirm those tool calls appear in `/api/realtime/evidence`.
