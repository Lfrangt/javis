...
- File workflows: current build supports list/search/summarize/Q&A, guarded by-type organization plans, batch rename previews, non-destructive copy-convert previews, explicit `confirm:true` plan apply, and Level 3 create/copy/move/rename actions; next, richer semantic converters and app-specific file result checks.
