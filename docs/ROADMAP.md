...
- Ongoing screen context with privacy zones: current build supports global private/clear screen mode plus local app/window/browser-host/region privacy rules in `/api/screen/privacy`; app/window/browser-host exclusion rules block screen image use in server-side quick/observe/vision/Realtime preflight model context, while region rules are persisted and exposed as pending renderer pixel-mask enforcement, and the renderer refuses to push full Realtime screen images when region rules exist but pixel masking is not implemented; next, apply selectable region masking in the renderer before Realtime screen image push.
