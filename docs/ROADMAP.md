...
- Realtime voice evidence monitor: current CUI option `V. Watch Realtime voice evidence` watches the real renderer/WebRTC evidence checklist, including structured status/phase/blocker output, renderer-recorded SDP offer/answer evidence, current-vs-injected work-progress sequence sync, a guided `/api/realtime/dogfood/drill` checklist, recent sanitized Realtime tool-call metadata, work-handoff evidence for `get_work_handoff`, shortcut list/candidate/save/forget evidence, and the `/api/realtime/dogfood` manual runbook. Realtime voice can also call `get_realtime_evidence` to explain whether live WebRTC/progress is connected, what is blocked, and the next dogfood step. CUI option `D. Start Realtime dogfood drill` and `/api/realtime/dogfood/start` explicitly summon the pet and can schedule a short local read-only progress sample after the renderer reports a live voice session. `/api/work/next` uses the same manual start path when the blocker is `needs_live_session` and now carries a structured dogfood guide with start, monitor, prompt, and expected-evidence instructions; this action is manual-only so unattended autopilot never starts microphone/live voice, and no desktop pet diagnostics are added.
