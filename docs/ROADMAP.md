...
- Task memory scoped to the local machine: current build supports explicit local memories plus a controllable local inferred learning profile with pause/resume, prompt inclusion, exclusions, deletion, promotion to memory, routing evidence, reviewable skill draft generation, and explicit UI demonstration records available from API/CUI/Realtime voice that turn sanitized app/browser/accessibility captures into manual-preview playbooks, safe replay plans, confirmation-gated replay runs, and confirmation-gated local skill drafts; next, richer retrieval inside workflows and automatic recall of proven demonstration skills or shortcuts.
