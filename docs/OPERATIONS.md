...
When routed browser work fails because no supported browser window is readable, work-next surfaces `browser_recovery:open_supported_browser`. Preview mode only reports the local `open_app` plan; execution opens or focuses Google Chrome through the normal local action policy, then the browser task can be retried without asking which window to use.
