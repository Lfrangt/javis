...
Inside `npm run voice:chat`, `/status`, `/handoff`, `/next`, and `/history` are read-only loop commands. `/status` defaults to the lightweight `/api/pet/status` payload, `/next` defaults to `/api/work/next?compact=true`, and `/handoff` plus `/history` read the voice-ready handoff/history directly, then print compact operator output without routing a task, recording a voice session turn, starting microphone capture, using Realtime, or exposing raw logs on the desktop pet. Use `/help` to list the loop commands; add `--full-status`, `--full-next`, or `--full` when you intentionally want full diagnostics.
