...
In the interactive CUI, option `1B` prints the same plan and can open the OpenAI API billing page after explicit confirmation. Option `SG` prints the OpenAI spend guard: hard lock, daily budget, unattended budget, blocked count, egress guard state, and recent blocked/allowed attempts. The recovery plan never starts microphone capture and keeps `/api/voice/command` / `npm run voice:chat` as the local fallback while billing or key changes are pending. The same recovery payload now includes `retryPolicy`, which says whether a no-mic provider probe is due, cooling down, or already running, plus whether the local fallback should be used until the next probe. Provider probes can consume OpenAI API quota, so the default is zero-spend: `JAVIS_OPENAI_HARD_SPEND_LOCK=true`, `JAVIS_OPENAI_CLOUD_MODE=off`, `JAVIS_OPENAI_DAILY_REQUEST_LIMIT=0`, `JAVIS_OPENAI_EGRESS_GUARD=true`, and unattended/autopilot/unscoped OpenAI egress paths are blocked. Local voice-command intake recognizes phrases like `我已经充值好了，帮我重试实时语音 provider probe，先不要开麦` and routes them to the same no-mic provider-probe preview path; actual execution still requires disabling the hard spend lock, setting a positive daily limit, restarting, and phrase-confirming one request.
