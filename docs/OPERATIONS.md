...
Inside `npm run voice:chat`, `/status`, `/app`, `/browser`, `/browse`, `/handoff`, `/next`, and `/history` are read-only loop commands. `/status` defaults to the lightweight `/api/pet/status` payload, `/app` reads `/api/mac/context` plus a capped Accessibility outline from `/api/accessibility/tree`, `/browser` reads `/api/browser/context` plus a capped `/api/browser/page` summary, `/browse [intent] <task>` previews the existing `/api/browser/workflow` lane over the current page, `/next` defaults to `/api/work/next?compact=true`, and `/handoff` plus `/history` read the voice-ready handoff/history directly, then print compact operator output without routing a task, recording a voice session turn, starting microphone capture, using Realtime, or exposing raw logs on the desktop pet. `/open <url or search>` is preview-only by default and routes through `/api/voice/command`; start the loop with `--run` only when you want browser workflow/open commands to execute through the normal local action policy. Use `/help` to list the loop commands; add `--full-status`, `--full-next`, or `--full` when you intentionally want full diagnostics.

Inside the same loop, `/agent <task>` previews `/api/autonomy/run` for that task. The chat-loop default is a short preview capped at 4 autonomy steps, enough for route, local learning evidence, safe observation, and one work-next preview without the slower progress/recovery scan. Add `--full-agent`, `--full`, or `--agent-steps <n>` when you intentionally want a deeper autonomy pass. It remains no-mic and preview-only, and summarizes `agencyPlan` without direct shell/UI execution. Use `npm run autonomy -- --task "<task>"` for the full non-interactive preview, or `npm run autonomy:run -- --task "<task>"` when you explicitly want one bounded step to run through the existing router, action policy, approval queue, and worker/recovery gates. In the CUI, use `AG` to preview and `AR` to preview then type `RUN`.
