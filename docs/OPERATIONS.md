...
When the compact pet is parked and Realtime is blocked or warning, its fallback click target is the terminal local voice loop rather than the expanded desktop panel. `/api/pet/status` exposes `localVoice.interaction.capsuleClick:"open_local_voice_loop"` and `/api/voice/open-local-loop`; the renderer uses that to open `npm run voice:chat` in Terminal while keeping the desktop pet compact. This path starts no microphone, uses no Realtime session, and keeps the heavier diagnostics in the CUI/API.
