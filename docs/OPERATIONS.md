...
When the compact pet is parked and Realtime is blocked or warning, its fallback click target is the quiet local typed intake path, not a Terminal loop or expanded diagnostic dashboard. `/api/pet/status` exposes `localVoice.interaction.capsuleClick:"open_local_input"` plus `endpoint:"/api/voice/command"` and `opensTerminal:false`; the renderer previews `/api/voice/standby` without executing the standby primary action, then focuses the local input. Operators can still deliberately open the continuous terminal loop through `npm run voice:open` or `/api/voice/open-local-loop`; that opener checks for an existing `local-voice-command-dogfood.mjs --chat` process and debounces recent opens, so explicit loop starts reuse/focus the existing window instead of stacking Terminal windows. This keeps the desktop pet compact, starts no microphone, uses no Realtime session, and keeps the heavier diagnostics in the CUI/API.
