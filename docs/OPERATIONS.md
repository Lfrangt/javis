...
In the interactive CUI, option `1B` prints the same plan and can open the OpenAI API billing page after explicit confirmation. Option `SG` prints the OpenAI spend guard: hard lock, emergency runtime lock, daily budget, unattended budget, blocked count, one-request lease state, egress guard state, runtime key-env isolation state, child-process credential guard state, and recent blocked/allowed attempts. Option `SS` or `npm run openai:sentinel` prints the resident OpenAI spend sentinel: it watches only local guard state on an interval and reports whether the current process is clear, warning, or breach without calling OpenAI, creating a lease, starting voice, or launching workers. Option `SI` or `npm run openai:incident` prints the local spend incident report for questions like "why did API quota move if I did not test": it reconciles local allowed/blocked spend-guard records, egress guard, runtime key-env isolation, child-process credential guard, and recent local OpenAI audit events without calling OpenAI or querying the billing dashboard. Option `SL` enforces zero-spend lockdown: it first calls `/api/openai/zero-spend-lockdown` so the current resident clears active one-request leases, removes OpenAI credential env from `process.env`, blocks any later current-process spend checks, and asks the renderer to stop an existing Realtime WebRTC voice session; then it rewrites `.env` zero-spend values and restarts the resident without deleting `OPENAI_API_KEY`. The recovery plan never starts microphone capture and keeps `/api/voice/command` / `npm run voice:chat` as the local fallback while billing or key changes are pending. The same recovery payload now includes `retryPolicy`, which says whether a no-mic provider probe is due, cooling down, or already running, plus whether the local fallback should be used until the next probe. Provider probes can consume OpenAI API quota, so the default is zero-spend: `JAVIS_OPENAI_HARD_SPEND_LOCK=true`, `JAVIS_OPENAI_CLOUD_MODE=off`, `JAVIS_OPENAI_DAILY_REQUEST_LIMIT=0`, `JAVIS_OPENAI_EGRESS_GUARD=true`, `JAVIS_OPENAI_REQUIRE_SPEND_LEASE=true`, `JAVIS_OPENAI_CHILD_ENV_GUARD=true`, `JAVIS_OPENAI_RUNTIME_KEY_ISOLATION=true`, and unattended/autopilot/unscoped OpenAI egress plus runtime/child-process key inheritance paths are blocked. The runtime key isolation keeps the `.env` key on disk but removes OpenAI credential variables from the resident's general `process.env` after startup; only the guarded OpenAI request functions retain the key for an explicitly leased request. The child env guard also strips protected OpenAI/JAVIS credential keys from MCP stdio server config env blocks before those servers start, while preserving non-secret env values. Local voice-command intake recognizes phrases like `我已经充值好了，帮我重试实时语音 provider probe，先不要开麦` and routes them to the same no-mic provider-probe preview path; actual execution still requires disabling the hard spend lock, setting a positive daily limit, restarting, creating a short-lived one-request spend lease with the exact phrase, and executing before that lease expires.
