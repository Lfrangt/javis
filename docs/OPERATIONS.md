...
Inside `npm run voice:chat`, `/status`, `/handoff`, `/next`, and `/history` are read-only loop commands. They query `/api/status`, `/api/work/handoff`, `/api/work/next`, and `/api/voice/history` directly, then print compact operator output without routing a task, recording a voice session turn, starting microphone capture, using Realtime, or exposing raw logs on the desktop pet. Use `/help` to list the loop commands.

Inside the same loop, `/agent <task>` previews `/api/autonomy/run` for that task. It is still no-mic and preview-only: route, local learning evidence, safe observation, work-next preview, progress verification, recovery scan, and `agencyPlan` are summarized without direct shell/UI execution. Use `npm run autonomy -- --task "<task>"` for the same non-interactive preview, or `npm run autonomy:run -- --task "<task>"` when you explicitly want one bounded step to run through the existing router, action policy, approval queue, and worker/recovery gates. In the CUI, use `AG` to preview and `AR` to preview then type `RUN`.
