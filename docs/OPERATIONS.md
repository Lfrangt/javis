...
Inside `npm run voice:chat`, `/status`, `/browser`, `/handoff`, `/next`, and `/history` are read-only loop commands. `/status` defaults to the lightweight `/api/pet/status` payload, `/browser` reads `/api/browser/context` plus a capped `/api/browser/page` summary, `/next` defaults to `/api/work/next?compact=true`, and `/handoff` plus `/history` read the voice-ready handoff/history directly, then print compact operator output without routing a task, recording a voice session turn, starting microphone capture, using Realtime, or exposing raw logs on the desktop pet. `/open <url or search>` is preview-only by default and routes through `/api/voice/command`; start the loop with `--run` only when you want it to execute through the normal local action policy. Use `/help` to list the loop commands; add `--full-status`, `--full-next`, or `--full` when you intentionally want full diagnostics.
