...
`npm run dogfood:productivity-live -- --suite --json` is the repeatable four-app evidence path. It saves a local archive under `Runtime/productivity-dogfood-archives/` by default and returns the archive path, safety markers, per-app status, workflow IDs, approval IDs, missing requirements, and recovery hints. Add `--execute --confirm` only for an intentional live Mac run; Mail remains draft-only and no send path is exposed.
