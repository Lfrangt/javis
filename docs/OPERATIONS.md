...
project and close old `npm run voice:chat` Terminal loops before loading the LaunchAgent. The
LaunchAgent starts `scripts/resident-launcher.cjs`, which starts Electron and waits for
`/api/health?lite=watchdog`; if the API does not become healthy, it terminates Electron and exits
non-zero so launchd/watchdog do not treat a shell app with no API as healthy. This prevents an
older or half-started process from keeping `JAVIS_API_PORT` unavailable while a newer LaunchAgent
instance appears to be running. `npm run voice:cleanup` only closes stale local voice Terminal loops
and their lock file; it does not stop the resident app.
...
`npm run setup:bundle` is the compact resident landing packet for daily use. It combines resident LaunchAgent state, setup blockers, permission checks, pet/notch state, Realtime recovery, local voice fallback, worker availability, action policy, learning/autopilot state, and the next safe action. It also prints `Zero-cost now` actions for local voice, the status board, capabilities, browser control, and work handoff so a blocked Realtime provider does not look like the only way forward. It is read-only: it does not open the microphone, call OpenAI, grant macOS permissions, open a browser, or mutate files.
...
The standby contract also includes `promptPack`: one next safe utterance plus a few examples ranked from existing local metadata. It can prioritize active-session check-ins, the local progress board (`打开本地进度看板，告诉我现在卡在哪里`), `继续刚才那个`, background progress, current browser/page controls, current-app UI controls, capability status, or local distillation depending on what JAVIS already knows. The same prompt pack is mirrored into `/api/pet/status`, wake handoff, `npm run voice:standby`, local `voice:chat` `/voice`, local `voice:chat` `/try`, natural prompt questions such as `我现在可以说什么`, and the compact compose input placeholder. The progress-board standby suggestion can be switched with `npm run voice:prompt-board:on`, `npm run voice:prompt-board:off`, CUI options `VP`/`VPON`/`VPOFF`, or `GET`/`POST /api/voice/prompt-settings`; this only changes local prompt suggestions and does not disable `/board` or the natural `progress_board` command. The same surfaces also expose `inputMode.mode=typed_local_intake`, `micDefault=off`, and the keyboard/system-dictation prompt, so fallback is described honestly as local typed intake rather than live voice. In fallback mode those prompts route through `/api/voice/command` and keep `startsMicrophone:false`, `usesRealtime:false`, `opensTerminal:false`, and model calls off.
...
In the interactive CUI, option `1B` prints the same plan and can open the OpenAI API billing page after explicit confirmation. Option `SG` prints the OpenAI spend guard: paranoid zero-spend, hard lock, emergency runtime lock, daily budget, unattended budget, blocked count, one-request lease state, egress guard state, runtime key-env isolation state, memory key-vault state, child-process credential guard state, and recent blocked/allowed attempts. Option `SS` or `npm run openai:sentinel` prints the resident OpenAI spend sentinel: it watches only local guard state on an interval and reports whether the current process is clear, warning, or breach without calling OpenAI, creating a lease, starting voice, or launching workers. Option `SI` or `npm run openai:incident` prints the local spend incident report for questions like "why did API quota move if I did not test": it reconciles local allowed/blocked spend-guard records, egress guard, runtime key-env isolation, memory key-vault state, child-process credential guard, and recent local OpenAI audit events without calling OpenAI or querying the billing dashboard. Option `SL` enforces zero-spend lockdown: it first calls `/api/openai/zero-spend-lockdown` so the current resident clears active one-request leases, removes OpenAI credential env from `process.env`, vaults the callable OpenAI key from current-process memory, blocks any later current-process spend checks, and asks the renderer to stop an existing Realtime WebRTC voice session; then it rewrites `.env` zero-spend values and restarts the resident without deleting `OPENAI_API_KEY`. The recovery plan never starts microphone capture and keeps `/api/voice/command` / `npm run voice:chat` as the local fallback while billing or key changes are pending. The same recovery payload now includes `retryPolicy`, which says whether a no-mic provider probe is due, cooling down, or already running, plus whether the local fallback should be used until the next probe. Provider probes can consume OpenAI API quota, so the default is paranoid zero-spend: `JAVIS_OPENAI_PARANOID_ZERO_SPEND=true`, `JAVIS_OPENAI_HARD_SPEND_LOCK=true`, `JAVIS_OPENAI_CLOUD_MODE=off`, `JAVIS_OPENAI_DAILY_REQUEST_LIMIT=0`, `JAVIS_OPENAI_EGRESS_GUARD=true`, `JAVIS_OPENAI_REQUIRE_SPEND_LEASE=true`, `JAVIS_OPENAI_CHILD_ENV_GUARD=true`, `JAVIS_OPENAI_RUNTIME_KEY_ISOLATION=true`, `JAVIS_OPENAI_MEMORY_KEY_VAULT=true`, and unattended/autopilot/unscoped OpenAI egress plus runtime/child-process key inheritance paths are blocked. The resident LaunchAgent injects those values only as safe defaults and then respects explicit `JAVIS_OPENAI_*` values from the project `.env`, while still keeping OpenAI credential keys out of the plist. This lets a user intentionally enter manual paid-provider setup without the LaunchAgent silently forcing `cloud=off` again. The runtime key isolation keeps the `.env` key on disk but removes OpenAI credential variables from the resident's general `process.env` after startup; the memory key vault also withholds the callable key from guarded OpenAI request functions while zero-spend mode is active. Local status surfaces therefore distinguish `hasOpenAiKey` from `openAiKeyAvailableForCalls`; `.env` can contain a key while callable spend remains unavailable. The child env guard also strips protected OpenAI/JAVIS credential keys from MCP stdio server config env blocks before those servers start, while preserving non-secret env values. Local voice-command intake recognizes phrases like `我已经充值好了，帮我重试实时语音 provider probe，先不要开麦` and routes them to the same no-mic provider-probe preview path; actual execution still requires disabling paranoid zero-spend, disabling the hard spend lock, setting a positive daily limit, restarting so the key is intentionally retained, creating a short-lived one-request spend lease with the exact phrase, and executing before that lease expires.
...
curl http://127.0.0.1:3417/api/voice/setup
```

`npm run voice:setup` is the voice go-live checklist. It combines macOS microphone permission, Realtime provider state, OpenAI spend guard, egress guard, one-request lease state, local fallback, and the final live renderer command in one read-only packet. It shows which steps are no-cost now, which step can spend one API request, and which step starts the microphone later. `npm run voice:doctor` is the quickest local voice entry diagnosis. It combines voice standby, wake handoff, OpenAI spend sentinel, local voice latency, and resident health in one read-only report, so an operator can see whether the right path is live Realtime, local no-mic fallback, wake/hotkey repair, or explicit paid-provider unlock. Natural local transcripts such as `实时语音为什么连不上，是什么意思？` route to the same recovery guide. These checks read Realtime provider health, the OpenAI spend guard, the egress guard, the one-request lease state, microphone status, and the local fallback entry point, but they do not call OpenAI, create a spend lease, start microphone capture, create a Realtime session, open a browser, open Terminal, or capture the screen.

Before opening a real microphone session, preview the no-mic provider probe. Preview mode makes no OpenAI request. It also prints `Key sync`, a short irreversible fingerprint check that says whether the current `.env` `OPENAI_API_KEY` is present, whether the running resident loaded the same key, and whether a restart is required after replacing the key. Confirmed run mode creates a renderer WebRTC offer without `getUserMedia`, calls the same OpenAI Realtime provider path with `probe=true`, records HTTP status/error evidence, and closes immediately, but it is intentionally high friction:

```bash
npm run config -- --print-realtime-provider-probe
npm run openai:spend
npm run openai:sentinel
...
The global tap-to-summon hotkey uses the same no-cost path by default. `/api/window/state`, `/api/pet/status`, and `/api/setup/recovery-bundle` expose `tapToSummon.currentAction`; the default is `open_compact_local_input`, with `startsMicrophone:false`, `usesRealtime:false`, `fallbackCallsOpenAi:false`, and `realtimeReadyMayStartRendererVoice:false`. If you intentionally want the hotkey to wake Realtime directly later, opt in with `JAVIS_TAP_TO_REALTIME=true` or `JAVIS_SUMMON_REALTIME=true`, then still unlock OpenAI spend through the normal hard-lock, budget, phrase, and one-request lease gates.
...
You can ask about wake behavior directly. Transcripts such as `我喊贾维斯为什么没反应`, `唤醒词状态怎么样`, `wake engine 开着吗`, and `怎么喊你` route to `wake_status`. This reads the same trigger-only wake state as `/api/wake/status` and `voice:chat` `/wake`: wake words, pending trigger TTL, last trigger, external `JAVIS_WAKE_ENGINE_CMD` status, and whether the next handoff should use Realtime or local no-mic intake. It is a diagnostic read only. It does not start the wake engine, does not start microphone capture or always-on listening, does not create a Realtime session, does not call OpenAI, does not open Terminal, and does not execute actions.

You can ask whether the resident is alive directly. Transcripts such as `你还活着吗`, `常驻服务状态怎么样`, `自恢复开了吗`, `watchdog 开着吗`, and `is JAVIS still running?` route to `resident_health`. This reads the same LaunchAgent and watchdog state as `/api/resident/status` and `voice:chat` `/resident`; it does not call OpenAI, create a spend lease, start microphone capture, create a Realtime session, capture screen, start workers, open Terminal, or mutate files.
...
You can ask for the local progress board directly. Transcripts such as `打开本地进度看板，告诉我现在卡在哪里`, `后台状态板怎么样`, `看板现在写什么`, and `progress board status` route to `progress_board`. This reads the same `/api/progress-board` semantic source as the HTML status board and returns a compact board summary, recovery candidate, blocker rows, next actions, and safety flags. It does not open the board window, call OpenAI, create a spend lease, start microphone capture, create a Realtime session, start workers, execute actions, capture screen, or mutate files.

To quiet the standby prompt without disabling the board command, run `npm run voice:prompt-board:off`; use `npm run voice:prompt-board:on` to turn it back on and `npm run voice:prompts` to inspect the current local setting. These commands only write JAVIS runtime prompt settings.
...
Browser checks also work as natural local voice commands. `浏览器准备好了吗，默认会看哪个窗口？` routes to `browser_readiness` and only reads the default target/bridge recovery packet. `读一下当前网页` routes to `browser_page` and reads the current page through `/api/browser/page`. `当前网页有哪些按钮和输入框？` routes to `browser_dom` and reads visible controls through `/api/browser/dom`. Browser chrome-control phrases such as `刷新当前网页`, `后退一页`, `前进一页`, `打开新标签页`, `关闭当前标签页`, and `选中地址栏` route through the browser workflow act lane with a local fallback plan first. Preview mode does not press browser shortcuts, call a model, start microphone capture, use Realtime, or open Terminal; use `execute:true` or CLI `--run` only when you want the browser action to run through the normal local action policy. These paths do not ask the user which window to share; the readiness path also avoids page text and page JavaScript. If Chrome's Apple Events window metadata is temporarily unavailable but the local Chrome DevTools bridge has a page target, JAVIS treats that CDP target as the default browser context so page, DOM, and guarded browser-control lanes remain ready without a window picker.
...
For long-running work, use the local visual board instead of reading chat logs:

```bash
npm run board
npm run board -- --no-open
curl http://127.0.0.1:3417/api/progress-board
```

`npm run board` first reads the public sanitized `/api/progress-board` endpoint with a short resident-startup retry, prints a compact terminal summary, then opens `docs/javis-status-board.html`. Use `npm run board -- --no-open` when you only want the summary or are testing from automation. `docs/javis-status-board.html` refreshes every few seconds and shows the current product nodes in plain Chinese: desktop pet, Realtime voice, local input fallback, zero-cost local actions, the current work-next recovery candidate, browser control, OpenAI spend safety, background work, and overnight readiness. Its "最近工作" timeline is generated from the local audit tail as sanitized event labels, sources, statuses, and ages rather than fixed copy, and the header shows the latest `/api/progress-board` render time so slow refreshes are visible. The backing `/api/progress-board` endpoint is intentionally public but sanitized, reads independent sections in parallel, and uses a short per-section timeout so one slow browser/overnight/work-next check does not freeze the board. It does not return the local API token, OpenAI key, screen images, clipboard text, browser page text, raw Accessibility nodes, raw logs, or user text, and it does not call OpenAI, start the microphone, use Realtime, start workers, or execute computer actions.

The board also embeds the same read-only Realtime go-live checklist exposed by `npm run voice:setup`: microphone permission, no-mic provider probe preview, one-request manual provider probe, and final mic-confirmed live start. The "零费用现在可做" panel mirrors the setup bundle's safe local actions for typed/no-mic input, status board, capabilities, browser control, and work handoff; each card is marked no-cost/read-only and is verified to avoid OpenAI, microphone, Realtime, workers, computer actions, and file mutations. The "下一步恢复" panel mirrors the current work-next preview and labels whether the candidate needs user presence, can spend API, can start the microphone, can start workers, or can execute actions; the panel itself remains preview-only. This is the preferred local visual surface when voice is stuck, because it explains the blocker without opening Realtime, creating a spend lease, calling OpenAI, starting workers, executing actions, or starting microphone capture.
...
JAVIS_EVAL_LIVE_BROWSER_ACTIONS=true npm run eval -- --only=voice-command
...
It also registers a tap-to-summon hotkey, defaulting to `Alt+Space` (`Option+Space` on Mac). Pressing it parks the capsule at the notch/Dynamic Island position and opens the compact `compose` input immediately, so the user can type or dictate a local no-mic turn without waiting for a microphone/provider path. Change the key with `JAVIS_SUMMON_HOTKEY` or `JAVIS_TAP_HOTKEY`, or set either value to `false` to disable it. Direct Realtime wake from this hotkey is off by default; it requires `JAVIS_TAP_TO_REALTIME=true` or `JAVIS_SUMMON_REALTIME=true` plus the normal explicit OpenAI spend unlock flow.
...
`/api/browser/benchmarks` and CUI option `G. Show browser workflow benchmarks` run preview-only fixture checks for browser summarize, action extraction, local act search fallback, DOM action safety contracts, submit-like DOM execute gates, form-fill draft redaction, compare/search preview, review-result preview, and research continuation metadata. They do not open a live browser page, execute browser actions, call a model, store raw fixture page text, or start voice; use them before broadening browser automation or debugging browser workflow regressions.

`/api/browser/control/status` and `npm run browser:control` expose the current browser chrome-control capability map: back, forward, reload, new tab, close tab, focus address bar, open URL, and search. The status call is read-only and safe for the local board/CUI/voice: it does not start a browser, ask which window to use, execute browser actions, run page JavaScript, read page text, call OpenAI, start microphone capture, or create a spend lease. Use the POST `/api/browser/control` path only after the user has clearly asked to execute one browser action through the local action policy.
