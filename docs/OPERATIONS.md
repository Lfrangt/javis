...
Realtime evidence now groups those UI-demonstration calls separately. The monitor should show the observed actions, whether replay stayed preview-only and re-observed live UI, whether a skill draft was produced, whether save/run was blocked by a confirmation gate, and whether no screenshots or raw clipboard text were stored.
