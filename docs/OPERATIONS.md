...
When the compact pet is parked and Realtime is blocked or warning, its fallback click target is the terminal local voice loop rather than the expanded desktop panel. `/api/pet/status` exposes `localVoice.interaction.capsuleClick:"open_local_voice_loop"`, and the renderer executes the current voice standby primary action through `POST /api/voice/standby`; today that opens `npm run voice:chat` in Terminal through `/api/voice/open-local-loop`. This keeps the desktop pet compact, starts no microphone, uses no Realtime session, and keeps the heavier diagnostics in the CUI/API.
