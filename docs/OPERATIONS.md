...
The global tap-to-summon hotkey uses the same no-cost path when Realtime is blocked. `/api/window/state`, `/api/pet/status`, and `/api/setup/recovery-bundle` expose `tapToSummon.currentAction`; in zero-spend fallback it is `open_compact_local_input`, with `startsMicrophone:false`, `usesRealtime:false`, and `fallbackCallsOpenAi:false`.
