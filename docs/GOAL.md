...
- Quiet desktop layer: support class/presentation mode where the pet can disappear persistently while the resident server, menu bar, hotkeys, background work, and recovery paths keep running.
...
- Extreme cloud-spend safety: an API key may exist locally, but cloud spend must default to paranoid zero-spend. The resident must force cloud off, zero budget, hard-lock on, renderer/autopilot cloud off, guarded OpenAI network egress, runtime key-env isolation, zero-spend memory key vaulting, child-process credential redaction, and inline key-env injection blocking unless the user explicitly disables `JAVIS_OPENAI_PARANOID_ZERO_SPEND`. Even after that, any intentional OpenAI request must require hard-lock off, positive budget, exact phrase confirmation, a short-lived one-request lease, and local audit evidence before the request can leave the resident or any background worker can inherit credentials. A runtime emergency lockdown must also be available to clear active leases, vault the callable key from current-process memory, and block current-process spend checks immediately before config reload or restart.
...
- Multi-agent capacity preflight and wave routing: support a user-facing 20-agent request as a no-side-effect planning step that reports safe concurrency, worker readiness, parallel waves, blocked lanes, and write-conflict serialization before any worker can start; when tasks are supplied for routing, route only a selected safe wave by default, persist the full plan under the local runtime, and resume remaining waves/serial queues by plan id after a restart.
