...
- Next verifiable engineering task: dogfood shortcut recall plus save/forget controls from a real renderer/WebRTC voice session, with the CUI evidence monitor open until the spoken progress answer, shortcut tool calls, confirmation gate, save/forget events, and routed recall are visible in local evidence.
