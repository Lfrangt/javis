...
- Extreme cloud-spend safety: an API key may exist locally, but cloud spend must default to zero and require hard-lock off, positive budget, exact phrase confirmation, a short-lived one-request lease, guarded OpenAI network egress, runtime key-env isolation, child-process credential redaction, inline key-env injection blocking, and local audit evidence before any OpenAI request can leave the resident or any background worker can inherit credentials.
