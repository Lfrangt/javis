...
- Multi-agent capacity preflight and wave routing: support a user-facing 20-agent request as a no-side-effect planning step that reports safe concurrency, worker readiness, parallel waves, blocked lanes, and write-conflict serialization before any worker can start; when tasks are supplied for routing, route only a selected safe wave by default and keep the remaining waves/serial queue recoverable.
