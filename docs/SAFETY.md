...
- Local skill recall (`search_local_skills`, `/api/skills/local`, or `/api/learning/skills`) is read-only. It extracts short summaries from local `SKILL.md` files and can add matching procedures to task context, but it does not execute the skill, approve actions, install tools, or expand permissions.
