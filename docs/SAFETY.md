...
- Browser form-fill drafts (`run_browser_workflow` with `intent:"fill_draft"` or `/api/browser/fill-draft`) match supplied fields to visible DOM controls first. They preview by default, block sensitive fields such as passwords and card numbers, require `confirm:true` for execution, and never submit the form by themselves.
