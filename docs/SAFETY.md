...
- Browser workflows reuse the read-only page reader and route output to quick/background lanes. `search` and `compare` workflows may navigate the browser to Google result pages through the guarded browser-control lane, then read those result pages and extract candidate result links. `review_result` may open one explicit URL or selected result link through the guarded `open_url` path, then read the target page. These workflows do not click page controls, type into arbitrary fields, submit forms, or make account changes by themselves.
