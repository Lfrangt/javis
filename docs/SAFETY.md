...
- Pet status is deliberately smaller than presence. `/api/pet/status` is for the always-on desktop capsule and omits raw screen images, runtime paths, model lists, memory/learning details, routing history, workflow logs/results, and notification bodies. Use `/api/status`, CUI, or Realtime tools for operator diagnostics.
