...
- OpenAI API key presence does not grant spend permission. Defaults are zero-spend: `JAVIS_OPENAI_HARD_SPEND_LOCK=true`, `JAVIS_OPENAI_CLOUD_MODE=off`, `JAVIS_OPENAI_DAILY_REQUEST_LIMIT=0`, unattended budget `0`, and exact-phrase confirmation required for any intentional cloud request after the hard lock is disabled. `npm run openai:lockdown` or `npm run openai:zero` restores those zero-spend values, keeps the key in place, enables the OpenAI egress guard, strips OpenAI credential environment variables from the runtime `process.env`, background child processes, and MCP stdio server env blocks, blocks inline `OPENAI_API_KEY=...` command injection by default, and restarts the resident so the running process reloads the lock.
