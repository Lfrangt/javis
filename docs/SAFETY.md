...
- OpenAI API key presence does not grant spend permission. Defaults are zero-spend: `JAVIS_OPENAI_HARD_SPEND_LOCK=true`, `JAVIS_OPENAI_CLOUD_MODE=off`, `JAVIS_OPENAI_DAILY_REQUEST_LIMIT=0`, unattended budget `0`, and exact-phrase confirmation required for any intentional cloud request after the hard lock is disabled. The resident OpenAI spend sentinel watches only local guard state on an interval and surfaces clear/warning/breach status through `/api/openai/spend-sentinel`, CUI `SS`, and `npm run openai:sentinel` without calling OpenAI or creating a lease. `npm run openai:lockdown` or `npm run openai:zero` restores zero-spend values, keeps the key in `.env`, first activates `/api/openai/zero-spend-lockdown` in the running resident to clear active one-request leases, remove OpenAI credential env from `process.env`, vault the callable key from current-process memory, and block current-process spend checks immediately, then restarts the resident so the `.env` lock is reloaded. Runtime surfaces distinguish `hasOpenAiKey` from `openAiKeyAvailableForCalls`; a key can be configured locally while `JAVIS_OPENAI_MEMORY_KEY_VAULT=true` prevents any callable OpenAI key from being retained during zero-spend mode. The same policy enables the OpenAI egress guard, strips OpenAI credential environment variables from background child processes and MCP stdio server env blocks, and blocks inline `OPENAI_API_KEY=...` command injection by default.
