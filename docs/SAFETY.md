...
- Demonstration replay planning (`plan_ui_demonstration_replay` or `/api/demonstrations/:id/replay/plan`) is preview-only. It converts completed explicit UI demonstrations into app workflow steps that must re-observe the live UI; it does not execute, approve, or reuse saved coordinates.
