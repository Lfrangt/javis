...
- OpenAI API key presence does not grant spend permission. Defaults are paranoid zero-spend: `JAVIS_OPENAI_PARANOID_ZERO_SPEND=true` forces `JAVIS_OPENAI_HARD_SPEND_LOCK=true`, `JAVIS_OPENAI_CLOUD_MODE=off`, `JAVIS_OPENAI_DAILY_REQUEST_LIMIT=0`, unattended budget `0`, renderer/autopilot cloud off, exact-phrase confirmation, one-request spend leases, egress guard, runtime key isolation, child env guard, and memory key vault even if a lower-level setting is accidentally loosened. The resident LaunchAgent also starts with these zero-spend variables, so `.env` can keep `OPENAI_API_KEY` without making it callable. The resident OpenAI spend sentinel watches only local guard state on an interval and surfaces clear/warning/breach status through `/api/openai/spend-sentinel`, CUI `SS`, and `npm run openai:sentinel` without calling OpenAI or creating a lease. `npm run openai:lockdown` or `npm run openai:zero` restores zero-spend values, keeps the key in `.env`, first activates `/api/openai/zero-spend-lockdown` in the running resident to clear active one-request leases, remove OpenAI credential env from `process.env`, vault the callable OpenAI key from current-process memory, and block current-process spend checks immediately, then restarts the resident so the `.env` lock is reloaded. Runtime surfaces distinguish `hasOpenAiKey` from `openAiKeyAvailableForCalls`; a key can be configured locally while `JAVIS_OPENAI_MEMORY_KEY_VAULT=true` prevents any callable OpenAI key from being retained during zero-spend mode. The same policy enables the OpenAI egress guard, strips OpenAI credential environment variables from background child processes and MCP stdio server env blocks, and blocks inline `OPENAI_API_KEY=...` command injection by default.
...
- Realtime tool-call evidence is diagnostic only. `/api/realtime/evidence` keeps a short in-memory list of `/api/tools/execute` metadata for dogfood: tool name, source, timing, success/error state, output type/size/keys, sanitized shortcut-tool fields, privacy-safe local learning profile summaries, and a derived `gapSummary` for the next manual dogfood step. It does not store full tool arguments, raw tool outputs, screen frames, page bodies, clipboard text, or permission grants.
...
- Pet status is deliberately smaller than presence. `/api/pet/status` is for the always-on desktop capsule and omits raw screen images, runtime paths, model lists, memory/learning details, routing history, workflow logs/results, collaboration ledgers, and notification bodies. Its `payloadContract` advertises the allowed top-level keys, forbidden top-level keys, byte budget, exact output bytes, and minimum headroom so tests can catch pet bloat before it breaks the always-on polling lane. Use `/api/status`, CUI, or Realtime tools for operator diagnostics.
...
- Incident reports are read-only local audit summaries. `/api/incident/report`, CUI `IR`, local voice `incident_report`, and `voice:chat` `/incident` return compact event metadata and current voice/window state only; they do not capture screen, read clipboard text, return browser page text, return full Accessibility trees, start microphone capture, create Realtime sessions, open Terminal, or execute actions. The OpenAI spend incident report (`/api/openai/spend-incident-report`, CUI `SI`, `npm run openai:incident`, or natural API quota voice questions) follows the same local-only rule: it reads spend-guard, egress-guard, child-env-guard, and compact audit metadata, but does not call OpenAI, create a spend lease, query the billing dashboard, start workers, or inspect the screen.
...
- Inbox route and process-next are preview-first. They require an explicit user command, CUI action, API request, or voice tool call to select an item, and `execute:true` still stops at `confirmation_required` until `confirm:true` is supplied. Confirmed execution processes only the selected or highest-priority open item, then reuses the normal Inbox router, queue, worker, and completion rules.
...
- Browser workflows reuse the read-only page reader and route output to quick/background lanes. `act` workflows first try deterministic local planning for safe search/open-url/fill/click requests; when zero-spend mode is active and no safe local plan exists, they stop locally instead of invoking a model planner. `search` and `compare` workflows may navigate the browser to Google result pages through the guarded browser-control lane, then read those result pages and extract candidate result links. `review_result` may open one explicit URL or selected result link through the guarded `open_url` path, then read the target page. `research` may open several explicit URLs or search-result links in sequence, synthesize their read-only snapshots, and persist structured continuation/recovery actions for prepared preview links, unvisited result links, failed pages, or follow-up links found on reviewed pages. These continuation actions are inert API arguments until a later explicit workflow or work-next call runs them; they do not click page controls, type into arbitrary fields, submit forms, or make account changes by themselves.
- Eval/manual-probe browser action sources are preview-only by default. `eval_` and `manual_probe_` sources that would touch the real browser are blocked unless `JAVIS_EVAL_LIVE_BROWSER_ACTIONS=true` is set for an attended live-browser dogfood run.
- Browser workflow benchmarks (`/api/browser/benchmarks` or CUI option `G`) are fixture-only and preview-only. They do not open live pages, execute browser actions, submit forms, call models, store raw fixture page text, or start voice; they verify workflow contracts, redaction, and continuation metadata before broader browser automation changes.
...
- App workflow benchmarks are preview-only. `/api/app/benchmarks` and the CUI verify generic app planning, explicit workflow preview, unsafe delete rejection, no app launches, no UI actions, no model calls, and no workflow-history writes before broader Mac app automation changes.
- Productivity workflows (`plan_productivity_workflow`, `run_productivity_action`, `/api/productivity/workflow`, or `/api/productivity/action`) plan Notes, Reminders, Calendar, and Mail work through staged action packs. Confirmed creation of notes, reminders, calendar events, or visible Mail drafts can use native macOS automation, but it is Level 3: it requires complete fields, `confirm:true`, local execution, control-mode permission, `allow.productivity_app`, macOS Automation/Accessibility permission, audit logging, and post-action observation. Sending email, sending calendar invitations/updates, deletes, and bulk edits are blocked or kept for human review. `/api/productivity/benchmarks` and CUI option `Y` are preview-only and verify these gates without app launches, UI actions, model calls, workflow-history writes, or message sends.
- Creative workflow benchmarks are preview-only. `/api/creative/benchmarks` and the CUI verify video/music planning, export confirmation gates, missing asset-path gates, and prompt previews without launching creative apps, running UI actions, calling models, writing workflow history, or mutating files.
...
- Knowledge workflows (`get_knowledge_vaults`, `search_knowledge_notes`, `run_knowledge_workflow`, or `/api/knowledge/*`) are local Markdown/Obsidian workflows. Vault discovery is metadata-only, search is read-only and returns bounded snippets/tags/wikilinks, and note creation/append/daily-note writes require `execute:true`, `confirm:true`, local execution, allowed file roots, and normal file write policy. Benchmarks write only inside a temporary fixture and clean it up.
- File workflow benchmarks are fixture-only and preview-only. `/api/files/benchmarks` and the CUI create a temporary project fixture, verify list/search/plan/apply-gate contracts, clean the fixture up, call no models, start no apps, and do not mutate user files.
...
- Work-next runs at most one selected step from the local workbench. It may open setup targets, summarize approvals, check progress/session state, process one Inbox item, or run one explicit routed-work recovery candidate through the same job/workflow/browser/file/app policy gates, but it never approves pending actions or batch-runs tasks by itself. Unattended autopilot may only use the routed-work path when the route's recommended candidate is an existing failed-job recovery action that already passes the same low-risk or trusted recovery checks; browser, clipboard-copy, and generic workflow continuation route candidates remain manual.
...
- `allow.productivity_app.allowedApps`: productivity apps that native note/reminder/calendar/mail-draft creation may target.
...
- Native productivity creation for Notes, Reminders, Calendar, and Mail drafts is Level 3, requires `JAVIS_ENABLE_LOCAL_EXEC=true`, and is governed by `allow.productivity_app`.
- Codex and Claude Code delegation is Level 3, requires `JAVIS_ENABLE_LOCAL_EXEC=true`, and is governed by `allow.code_agent`. By default, worker child processes do not inherit `OPENAI_API_KEY`, `OPENAI_ADMIN_KEY`, `OPENAI_ORG_ID`, `OPENAI_ORGANIZATION`, `OPENAI_PROJECT`, `JAVIS_OPENAI_SPEND_CONFIRMATION_PHRASE`, or `JAVIS_API_TOKEN`; command strings that try to set those variables inline are blocked unless `JAVIS_OPENAI_CHILD_ENV_GUARD=false` is explicitly configured.
