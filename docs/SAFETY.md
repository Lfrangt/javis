...
- Productivity workflows (`plan_productivity_workflow`, `run_productivity_action`, `/api/productivity/workflow`, or `/api/productivity/action`) plan Notes, Reminders, Calendar, and Mail work through staged action packs. Creating notes, reminders, calendar events, or email drafts requires complete fields and explicit confirmation. Sending email, sending calendar invitations/updates, deletes, and bulk edits are blocked or kept for human review. `/api/productivity/benchmarks` and CUI option `Y` are preview-only and verify these gates without app launches, UI actions, model calls, workflow-history writes, or message sends.
