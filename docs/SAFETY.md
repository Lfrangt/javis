...
- Pet status is deliberately smaller than presence. `/api/pet/status` is for the always-on desktop capsule and omits raw screen images, runtime paths, model lists, memory/learning details, routing history, workflow logs/results, collaboration ledgers, and notification bodies. Its `payloadContract` advertises the allowed top-level keys, forbidden top-level keys, and byte budget so tests can catch pet bloat. Use `/api/status`, CUI, or Realtime tools for operator diagnostics.
