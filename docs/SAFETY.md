...
- Realtime tool-call evidence is diagnostic only. `/api/realtime/evidence` keeps a short in-memory list of `/api/tools/execute` metadata for dogfood: tool name, source, timing, success/error state, output type/size/keys, sanitized shortcut-tool fields, and a derived `gapSummary` for the next manual dogfood step. It does not store full tool arguments, raw tool outputs, screen frames, page bodies, clipboard text, or permission grants.
