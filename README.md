...
- Quota-aware pet fallback: when Realtime provider health is blocked or warning, the compact desktop capsule expands only into a narrow local input strip instead of trying to start the microphone, spawning Terminal windows, or opening the full diagnostic panel. The resident launch agent starts from the project directory with Terminal voice loops disabled and clears stale `voice:chat` loops on restart. The explicit CUI/CLI terminal loop remains available through manually running `npm run voice:chat`; the API loop opener redirects to the pet input and ignores the legacy product env unless the developer-only `JAVIS_DEV_ALLOW_TERMINAL_VOICE_LOOP=true` gate plus `allowTerminal:true` and `confirmTerminal:true` are all supplied. `voice:chat` also has its own single-instance lock, so repeated starts reuse the existing loop instead of stacking Terminal windows.
