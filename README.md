...
- Voice standby prompt pack: `/api/voice/standby`, `/api/pet/status`, wake handoff, the CUI, and the compact compose input now share one short `promptPack` with the next safe utterance and a few local examples, so Realtime quota failures still leave a clear "what to say next" path without opening Terminal, starting microphone capture, or turning the pet into a dashboard.
