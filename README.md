...
- Local voice-command loop slash checks: `/status`, `/handoff`, `/next`, and `/history` read the resident state, voice-ready handoff, next workbench action, and sanitized voice history from inside `npm run voice:chat` without routing a task, starting microphone capture, or using Realtime.
- Bounded local autonomy entrypoints from `npm run autonomy`, `npm run autonomy:run`, CUI `AG`/`AR`, and `voice:chat` `/agent <task>`, so the no-mic assistant can inspect context, plan fallbacks, preview work-next/recovery, and only execute through existing policy gates after explicit run intent.
...
- Sanitized local voice-command history through `/api/voice/history` and `npm run config -- --print-voice-history`, storing only transcript previews, route ids, lane/status, and metadata summaries for recovery/debugging while omitting raw audio, screenshots, clipboard text, browser page bodies, and full Accessibility nodes.
