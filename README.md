...
- Local voice-command fallback through `/api/voice/command` and `npm run dogfood:voice-command`, so typed transcripts or future local STT can route work with metadata-only Mac context and dry-run a spoken acknowledgement without Realtime, microphone capture, screenshot payloads, clipboard text, default memory attachment, or accidental quick-lane cloud calls.
