...
- Quota-aware pet fallback: when Realtime provider health is blocked or warning, the compact desktop capsule keeps its tiny Dynamic Island shape and a click opens or reuses `npm run voice:chat` in Terminal through `/api/voice/open-local-loop` instead of trying to start the microphone again or stacking duplicate Terminal windows.
