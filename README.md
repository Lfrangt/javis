...
- Quota-aware pet fallback: when Realtime provider health is blocked or warning, the compact desktop capsule keeps its tiny Dynamic Island shape and a click opens the quiet local input path instead of trying to start the microphone or spawning Terminal windows. The explicit CUI/CLI terminal loop remains available through `npm run voice:open` or `/api/voice/open-local-loop`, with duplicate-window protection.
