...
- Quota-aware pet fallback: when Realtime provider health is blocked or warning, the compact desktop capsule expands only into a narrow local input strip instead of trying to start the microphone, spawning Terminal windows, or opening the full diagnostic panel. The explicit CUI/CLI terminal loop remains available through `npm run voice:chat`; the API loop opener now redirects to the pet input unless `JAVIS_ALLOW_TERMINAL_VOICE_LOOP=true`, `allowTerminal:true`, and `confirmTerminal:true` are all supplied, with duplicate-window protection behind that developer-only gate.
