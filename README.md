...
- Local user-distillation status pack from `/api/learning/distillation` and CUI, combining inferred habits, recent evolution, explicit UI demonstrations, skill shortcuts, local skills, privacy boundaries, prompt-injection risk, and confirmation-gated next actions without storing raw screenshots, clipboard text, or page bodies.
