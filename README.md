...
- Local voice-command fallback through `/api/voice/command` and `npm run dogfood:voice-command`, so typed transcripts or future local STT can route work with metadata-only Mac context plus optional bounded Accessibility outline, and dry-run a spoken acknowledgement without Realtime, microphone capture, screenshot payloads, clipboard text, full AX node payloads, default memory attachment, or accidental quick-lane cloud calls.
