...
- Natural no-mic progress-board check-ins: transcripts such as `打开本地进度看板，告诉我现在卡在哪里`, `后台状态板怎么样`, or `progress board status` now route to the read-only `progress_board` path, returning the same semantic board state as `/api/progress-board` without opening the board window, calling OpenAI, starting microphone capture, using Realtime, starting workers, or executing actions.
