...
- Read-only MCP server discovery, preview-only MCP workflow planning, and local MCP execution approval requests for Claude Desktop/Claude Code/Cursor/project JSON configs through API, CUI, and Realtime voice tools; env values and URL queries are redacted, server commands are never started, and MCP tools are never called during preview or approval creation.
...
Local Express service on 127.0.0.1:3417
  ...
  /api/mcp/workflow     -> preview which MCP server should handle a task or create a local approval request, without starting servers or calling MCP tools
