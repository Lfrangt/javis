...
- No-mic Realtime provider probe through CUI/API, so API key, project quota, model, voice, and the Realtime endpoint can be tested before starting microphone capture; default commands only preview, OpenAI spending is hard-locked by default, daily budget defaults to 0, and any intentional request requires turning off the hard lock plus typing the exact spend phrase.
...
- OpenAI spend firewall through the resident main process: zero-spend defaults keep the hard lock on and daily budget at 0; intentional cloud calls need the exact phrase plus a short-lived one-request local spend lease; unscoped `fetch`/`http`/`https` calls to OpenAI API/Realtime hosts are blocked unless they are inside a spend-guard reservation scope; background Codex/Claude/CLI child processes do not inherit OpenAI credential environment variables by default and inline `OPENAI_API_KEY=...` command injection is blocked. `/api/openai/egress-guard/probe` proves the network block without sending a real API request, and `/api/openai/child-env-guard/probe` proves the child-process credential block without starting a process.
- Natural OpenAI spend-status questions through local voice/no-mic intake, so questions like "为什么消耗了 API 额度" read the same local spend guard, blocked count, one-request lease, and egress firewall state without creating a spend lease, calling OpenAI, starting microphone capture, or using Realtime.
...
- Voice standby prompt pack: `/api/voice/standby`, `/api/pet/status`, wake handoff, the CUI, `/try`, natural `prompt_suggestions`, and the compact compose input now share one short context-aware `promptPack` with the next safe utterance and a few local examples. It ranks continuation, active/blocked work, browser activity, cached current-app UI, capability, and local distillation prompts from existing metadata, so Realtime quota failures still leave a clear "what to say next" path without opening Terminal, starting microphone capture, calling a cloud model, or turning the pet into a dashboard.
...
- Natural work-session notes: local voice/no-mic intake recognizes phrases such as `记到当前会话：...`, `给当前会话记一下...`, and `把...写进工作会话`, writing one local session note only when the command is executed. The same surface reports safety flags proving it does not start microphone capture, use Realtime, open Terminal, capture screen, read clipboard text, execute app actions, or mutate user files.
...
Use the terminal CUI to paste `OPENAI_API_KEY` locally. It hides the input, writes only to `.env`, and can restart the resident service so the key is loaded. A saved key is not passed to background child processes while `JAVIS_OPENAI_CHILD_ENV_GUARD=true`. Do not paste API keys into chat.
