...
- Scriptable CUI work-next runner through `npm run work:next` and `npm run work:run -- --action-id route:<id>`, so routed previews can be continued from terminal automation without opening the interactive CUI.
