...
- Local voice-command loop slash checks: `/status`, `/handoff`, `/next`, and `/history` read resident state, voice-ready handoff, next workbench action, and sanitized voice history from inside `npm run voice:chat` without routing a task, starting microphone capture, or using Realtime. `/status` and `/next` default to lightweight fast payloads, with `--full-status` / `--full-next` for deep diagnostics.
