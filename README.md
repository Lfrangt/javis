...
- OpenAI egress circuit breaker through the resident main process, so unscoped `fetch`/`http`/`https` calls to OpenAI API/Realtime hosts are blocked locally unless they are inside an explicit spend-guard reservation scope; `/api/openai/egress-guard/probe` proves the block without sending a real API request.
