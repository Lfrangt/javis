...
| `npm run dogfood:voice-command` | Local voice-command dogfood: sends a transcript through `/api/voice/command`, previews task routing with metadata-only Mac context, and dry-runs the spoken acknowledgement without Realtime | preview routes + routing records; no microphone, Realtime, screenshots, or clipboard text |
