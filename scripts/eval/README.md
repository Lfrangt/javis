...
| `npm run dogfood:voice-command` | Local voice-command dogfood: sends a transcript through `/api/voice/command`, previews task routing, and dry-runs the spoken acknowledgement without Realtime | preview routes + routing records; no microphone or Realtime |
